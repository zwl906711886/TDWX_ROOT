<template>
  <div style="padding: 10px">
    ...
    <div style="text-align: center">
      <i class="iconfont icon-liebiao"></i> &nbsp  &nbsp 批次记录
    </div>
    ...
    <el-table
      ...
      <el-table-column
          prop="curState"
          label="目前状态"
          :filters="[{ text: '未开放', value: '未开放' }, { text: '已开放', value: '已开放' }]"
          :filter-method="filterTag"
          filter-placement="bottom-end">
        <template #default="scope">
              <el-tag
              :type="scope.row.curState === '已开放' ? 'primary' : 'success'"
              disable-transitions>{{scope.row.curState}}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog title="提示" v-model="dialogVisible" width="30%">
      <el-form :model="form" label-width="120px">
        ...
        <el-form-item label="目前状态">
          <el-select  style="width: 80%" v-model="form.curState" placeholder="请选择">
            <el-option
  ...
  methods: {
    ...
    export2Excel() {
      require.ensure([], () => {
        ...
        const tHeader = ['批次', '开始时间','结束时间','目前状态'];
        // 上面设置Excel的表格第一行的标题
        const filterVal = ['batchName', 'startTime','endTime','curState'];
        // 上面的 goodsName、goodsId、是tableData里对象的属性
        const list = this.tableData;  //把data里的tableData存到list
        const data = this.formatJson(filterVal, list);
        export_json_to_excel(tHeader, data, '批次管理表');
      })
    },
    ...
    save() {
      ...
      if (this.form.id) {  // 更新
        request.put("/batch", this.form).then(res => {
        ...
        request.post("/batch", this.form).then(res => {
          console.log(this.form)
          console.log(res)
          if (res.code === '0') {
            this.$message({
    ...
    handleDelete(id) {
      console.log(id)
      request.delete("/batch/" + id).then(res => {
        if (res.code === '0') {
          this.$message({
            type: "success",
            message: "删除成功"
